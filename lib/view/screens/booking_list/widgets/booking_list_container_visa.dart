...
class BookingListContainerVisa extends StatelessWidget {
  const BookingListContainerVisa({super.key, required this.visa, required this.selectedTab});
    ...
    return Container(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            _buildRow('Code', value: visa.code),
            const SizedBox(height: 10),
            _buildRow('Name', value: visa.toName),
            const SizedBox(height: 10),
            _buildRow('Phone', value: visa.toPhone),
            const SizedBox(height: 10),
            _buildRow('Email', value: visa.toEmail),
            const SizedBox(height: 10),
            _buildRow('Role', value: visa.toRole),
            const SizedBox(height: 10),
            _buildRow('Appointment Date', value: visa.appointment),
            const SizedBox(height: 10),
            _buildRow('Travel Date', value: visa.travelDate),
            const SizedBox(height: 10),
            _buildRow('Price', value: visa.totalPrice),
            const SizedBox(height: 10),
            _buildRow('Country', value: visa.country ?? 'N/A'),
            const SizedBox(height: 10),
            _buildActionsRow(
              (){
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => BookingListDetailsScreen(
                      id: visa.id,
                      type: 'visa',
                      visa: visa,
                    ),
